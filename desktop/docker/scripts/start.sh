...
function runWebPageReplay() {
  ...
  record_pid=$!
  sleep $RECORD_WAIT
  if [ -n "$BASEOPTS" ] && [ -n "$REFOPTS" ] ; then
    echo $BROWSERTIME_RECORD --firefox.preference network.dns.forceResolve:127.0.0.1 --chrome.args host-resolver-rules="MAP *:$HTTP_PORT 127.0.0.1:$WPR_HTTP_PORT,MAP *:$HTTPS_PORT 127.0.0.1:$WPR_HTTPS_PORT,EXCLUDE localhost" --pageCompleteCheck "$WAIT_SCRIPT" "$BASEOPTS"
    $BROWSERTIME_RECORD --firefox.preference network.dns.forceResolve:127.0.0.1 --chrome.args host-resolver-rules="MAP *:$HTTP_PORT 127.0.0.1:$WPR_HTTP_PORT,MAP *:$HTTPS_PORT 127.0.0.1:$WPR_HTTPS_PORT,EXCLUDE localhost" --pageCompleteCheck "$WAIT_SCRIPT" "$BASEOPTS" "$@"
  else
      ...
      if [ $? -eq 0 ]
        then
          if [ -n "$BASEOPTS" ] && [ -n "$REFOPTS" ] ; then
            ...
            echo "Using BASE options: $BASEOPTS"
            ...
            echo "--------------------------------------------"
            echo "Using REF options: $REFOPTS"
            exec $BROWSERTIME --firefox.preference network.dns.forceResolve:127.0.0.1 --firefox.preference security.OCSP.enabled:0 --chrome.args host-resolver-rules="MAP *:$HTTP_PORT 127.0.0.1:$WPR_HTTP_PORT,MAP *:$HTTPS_PORT 127.0.0.1:$WPR_HTTPS_PORT,EXCLUDE localhost" --video --visualMetrics --pageCompleteCheck "$WAIT_SCRIPT" --connectivity.engine throttle --connectivity.throttle.localhost --connectivity.profile custom --connectivity.latency $LATENCY "$REFOPTS" "$@" &
            PID=$!
            wait $PID
